...
public class CartRestController {
	...
	public ResponseEntity<Cart> clearCart(@PathVariable(value = "cartId") int cartId) {
		Cart cart = cartService.getCartById(cartId);
	...
	@RequestMapping(value = "/plusQuantity/{productId}", method = RequestMethod.PUT)
	public ResponseEntity<Void> plusQuantity(@PathVariable(value = "productId") int productId) {
		Product product = productService.getProductById(productId); //product를 가져온다
		
		//현재 인증된 사용자 정보 가져오기
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		String username = authentication.getName();
		
		//유저의 정보와 그 유저의 cart와 cartItem들을 가져온다
		User user = userService.getUserByUsername(username);
		Cart cart = user.getCart();	
		CartItem cartItem = cartItemService.getCartItemByProductId(cart.getId(), productId);
		
		int q = cartItem.getQuantity();
		if(q < product.getUnitInStock()) { //상품의 quantity가 재고량보다 작다면 quantity+1
		cartItem.setQuantity(q+1);
		cartItem.setTotalPrice(product.getPrice() * cartItem.getQuantity());
		cartItem.setProduct(product);
		cartItem.setCart(cart);
		
		cartItemService.addCartItem(cartItem); //cartItem을 update한다

		return new ResponseEntity<>(HttpStatus.OK); //httpStatus에 OK를 넣어서 보낸다
		}else {
			//상품의 quantity가 재고량보다 많을 시 quantity를 증가하지 않고 status에 BAD_REQUEST를 넣어 보낸다
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
		}
	}
	
	@RequestMapping(value = "/minusQuantity/{productId}", method = RequestMethod.PUT)
